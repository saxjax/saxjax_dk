...
export class RedBlackTree<T> {
  ...
  findNode = (input: RedBlackNode<T>, nodeToCompareWith?: RedBlackNode<T>): RedBlackNode<T> | undefined => {
    if (nodeToCompareWith === undefined) {
      return undefined
    }
    if (this.isEqualTo(input.value, nodeToCompareWith.value)) {
      return nodeToCompareWith
    }
    if (this.isLessThan(input.value, nodeToCompareWith.value)) {
      return this.findNode(input, nodeToCompareWith.left) ?? nodeToCompareWith
    }
    if (this.isGreaterThan(input.value, nodeToCompareWith.value)) {
      return this.findNode(input, nodeToCompareWith.right) ?? nodeToCompareWith
    }
    return undefined
  }
