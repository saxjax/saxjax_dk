...
export class MathPageComponent {
  ...
  mathRules = [
    {name:"Matrix multiplication",formula:"AB=C -> C_{ij} = Sigma(n_l=1) a_{il}b_{lj}= +a_{i1}b_{1j}...+a_{in}b_{nj}(i=1...k;j=1...p)when order-A=(k x n) orderB(n x p)"},
    ...
    {name:"(R1)",formula:"when matrix contains both nonzero- and zeroe-rows, all zeo-rows apear under all nonzero-rows"},
    {name:"(R2)",formula:"the first nonzero element in any nonzero-row is of value 1: [0 0 1 4 3]"},
    {name:"(R3)",formula:"All elements directly below the first nonzero element are 0 , the column has only zeroes below first nonzero element"},
    {name:"(R4)",formula:"the first nonzero element in any nonzero-row is left of previous first nonzero elements: r1[1234] r2[0123] r3[0012] "},
    ...
    {name:"Identity matrix I",formula:"the diagonal matrix, containing only 1's in its diagonal: r1[100]r2[010]r3[001]"},
    {name:"(-1)",formula:"AI=A "},
    {name:"(-2)",formula:"IA=A "},
